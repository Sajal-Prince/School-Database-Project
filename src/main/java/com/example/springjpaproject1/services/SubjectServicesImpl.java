...
public class SubjectServicesImpl implements SubjectServices{
    ...
    public SubjectServicesImpl(SubjectRepositories subjectRepositories, ProfessorRepositories professorRepositories, StudentRepositories studentRepositories) {
    ...
    @Transactional
    @Override
    public ResponseEntity<?> editSubjectEntity(Long id, SubjectDTO subjectDTO) {
        SubjectEntities oldSubject = subjectRepositories.findById(id).orElseThrow(() -> new SubjectNotFoundException("Subject with that Id was not found"));

        if(oldSubject.getStudentEntities()!=null){
            for(StudentEntities student:oldSubject.getStudentEntities())
                student.getSubjectEntities().remove(oldSubject);
            oldSubject.getStudentEntities().clear();
        }

        if(oldSubject.getProfessor()!=null){
            oldSubject.getProfessor().getSubjectEntities().remove(oldSubject);
            oldSubject.setProfessor(null);
        }

        if(subjectDTO.getProfessorId()!=null){
            ProfessorEntities professorEntities = professorRepositories.findById(subjectDTO.getProfessorId()).orElseThrow(()->new ProfessorNotFoundException("Profess with that id not found"));
            oldSubject.setProfessor(professorEntities);
            professorEntities.getSubjectEntities().add(oldSubject);
        }

        if(subjectDTO.getStudentEntities()!=null){
            List<StudentEntities> studentEntities = studentRepositories.findAllById(subjectDTO.getStudentEntities());
            for(StudentEntities student:studentEntities) {
                student.getSubjectEntities().add(oldSubject);
                oldSubject.getStudentEntities().add(student);
            }
        }

        return ResponseEntity.ok("Subject was edited");
    }
